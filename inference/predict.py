...
class Predictor:
    ...
    def predict(self, student: Union[dict, pd.DataFrame], save: bool = True) -> dict:
        ...
        for target in TARGETS:
            ...
            if hasattr(model, "predict_proba"):
                ...
                confidences[target] = {cls: round(float(prob), 4) for cls, prob in zip(classes, probabilities)}
    ...
    def _build_response(self, raw_preds: dict, confidences: dict, profile: dict) -> dict:
        ...
        weighted_success = success_conf.get("high", 0.0) + 0.5 * success_conf.get("medium", 0.0)
        ...
        return {
            ...
            "recommended_game": self.game_recommender.recommend(profile, outcome),
            "game_library": self.game_recommender.build_game_library(profile, outcome),
